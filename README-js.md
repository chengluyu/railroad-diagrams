...
* `Options.ESCAPE_HTML` - if `true`, causes `.toText()` to replace "<". ">", '"', and "&" with their HTML-entity equivalents, so text diagram output can be placed in HTML files unchanged.  Defaults to `true`.
* `Options.USE_CSS_CLASSES` - if `true` (the default), class attributes like `class="terminal"` and `class="railroad-diagram"` are added to the SVG elements. If `false`, no class attributes are included, making the SVG fully self-contained and portable for use in applications like Adobe Illustrator or Inkscape that may not support CSS classes. Note that all styling is always applied via inline SVG attributes regardless of this setting, so the visual appearance is identical.
